...
class ApiModel2 extends CI_Model
{
	...
	public function updateData($data, $id)
	{
		$periode = date('Y-m-') . '01';
		$cBlth = date('m_Y/');
		$sql = "UPDATE pelayanan.baca_meter SET stand_ini = ?, stand_ini_awal = ?, pakai = ?, status_baca = ?, valid_koordinat = ?, foto = ?,  tanggal_baca = ?, tanggal_upload = NOW() WHERE periode = ? AND id_pelanggan = ? ";
		$this->db->trans_begin();
		foreach ($data as $da) {

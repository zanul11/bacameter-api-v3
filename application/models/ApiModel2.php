...
class ApiModel2 extends CI_Model
{
	...
	{
		$sql = "SELECT gol.denda as denda,rekening.periode,rekening.harga_air+rekening.biaya_retribusi+rekening.biaya_jasa_lingkungan+rekening.biaya_pemeliharaan+rekening.biaya_administrasi+rekening.biaya_materai+rekening.biaya_denda+rekening.biaya_ppn-rekening.diskon as total from pelayanan.rekening_air as rekening, pelayanan.golongan as gol where id_pelanggan=? AND kasir is NULL AND waktu_bayar IS NULL AND rekening.id_golongan=gol.id";
	...
	public function updateDataV3($data, $id)
	{
		...
		foreach ($data as $da) {
			...
			$foto = 'http://10.10.222.236:8084/images/' . $cBlth . $id . '/' . substr($da['txtFoto'], -27);
			$query = $this->db->query("SELECT id FROM pelayanan.pelanggan WHERE no_langganan = ? ", array($da['cIdPel']));
			$row = $query->row_array();
			$name_foto_survey = null;
			if ($da['txtFotoSurvey'] != null) {
			...
			if ($da['cKetWM'] == null) {
				$sql = "UPDATE pelayanan.baca_meter SET new_latitude = ?, new_longitude = ?,telepon = ?, alamat = ?,indikasi_kelainan = ?, foto_survey = ?, catatan_survey = ?, stand_ini = ?, stand_ini_awal = ?, pakai = ?, valid_koordinat = ?, foto = ?,  tanggal_baca = ?, tanggal_upload = NOW() WHERE periode = ? AND id_pelanggan = ? AND tanggal_baca IS NULL";
	...
	public function sinkronDataV3($data, $id)
	{
		$periode = date('Y-m-') . '01';
		$cBlth = date('m_Y/');
		$sql = "UPDATE pelayanan.baca_meter SET new_latitude = ?, new_longitude = ?, telepon = ?, alamat = ?, indikasi_kelainan = ?, foto_survey = ?, catatan_survey = ?, stand_ini = ?, stand_ini_awal = ? , pakai = ?, status_baca = ?, valid_koordinat = ?, foto = ?,  tanggal_baca = ?, tanggal_upload = NOW() WHERE periode = ? AND id_pelanggan = ? AND status_baca!='BACAMETER MANDIRI'";
		$this->db->trans_begin();
		foreach ($data as $da) {
			$query = $this->db->query("SELECT id FROM pelayanan.pelanggan WHERE no_langganan = ? ", array($da['cIdPel']));
			$row = $query->row_array();
			...
			$this->db->query($sql, array(($da['vcLatitudeNew'] != 'null') ? $da['vcLatitudeNew'] : null, ($da['vcLongitudeNew'] != 'null') ? $da['vcLongitudeNew'] : null, ($da['telepon'] != 'null') ? $da['telepon'] : null, ($da['alamat'] != 'null') ? $da['alamat'] : null, $da['cIndikasiKelainan'], $name_foto_survey, $da['cKetSurvey'], $da['nStIni'], $da['nStIni'], $da['nPakai'], $da['cKetWM'], $da['lValidLokasi'], base_url('images/') . $cBlth . $id . '/' . $da['txtFoto'], $da['dTglCatat'],  $periode, $row['id']));
		}
